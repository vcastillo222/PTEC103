...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $staff_pass = $_POST['staff_pass']; // plain text, no md5

    $stmt = $conn->prepare("SELECT * FROM staff WHERE staff_user = ? AND staff_pass = ?");
    $stmt->bind_param("ss", $staff_user, $staff_pass);
    $stmt->execute();
    ...
    if ($result && $result->num_rows === 1) {
        ...
        echo "<div style='color:red; text-align:center;'>Invalid staff credentials</div>";
    }
...
<form class="login" method="POST" action="staff.php">
    ...
    Password: <input type="password" name="staff_pass"><br><br>
